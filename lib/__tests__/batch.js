...
import {getOperationFunction, executeBatch, validationBatchPayload} from '../batch.js'
...
test('validationBatchPayload / missing requests', t => {
  t.throws(() => validationBatchPayload({}, new Set()), {message: 'requests is a required param (array)'})
})

test('validationBatchPayload / too many requests', t => {
  const requests = Array.from({length: 101}, (_, i) => ({operation: 'geocode', params: {q: i}, id: i}))
  t.throws(() => validationBatchPayload({requests}, new Set()), {message: 'requests must not contains more than 100 items'})
})

test('validationBatchPayload / unknown operation', t => {
  const requests = [{operation: 'foo', params: {q: 'bar'}, id: '1'}]
  t.throws(() => validationBatchPayload({requests}, new Set(['bar'])), {message: 'operation is unknown'})
})

test('validationBatchPayload / missing params', t => {
  const requests = [{operation: 'geocode', id: '1'}]
  t.throws(() => validationBatchPayload({requests}, new Set(['geocode'])), {message: 'params is required for each requests item'})
})
